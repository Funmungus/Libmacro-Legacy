...
{
	std::vector < Dispatcher::GrabThread > Dispatcher::_threads ;
	std::map < std::string, Grabber * > Dispatcher::_threadGrabs ;
	std::mutex Dispatcher::_threadInitLock ; 	// merge _enableThread?
// mutex Dispatcher::_resetLock ;
	std::thread Dispatcher::_enableThread ;
	std::mutex Dispatcher::_enableLock ;
...
//		_threads.clear ( ) ;
	...
	unsigned char bit_retrieval [ KEY_CNT / 8 + 1 ] ;
	const unsigned int MODBIT_SIZE = ( sizeof ( unsigned char ) ) * getMaxModVal ( ) / 8 ;

	const unsigned int ModKeysLen =
			2 * Mods::MODSET_LENGTH ;

	void modify ( Mods * cur, unsigned char * keybit_values )
	{
...
keyback.setKey ( 0 ) ;
							...
							if ( ! trigger ( & keyback, & InternalMods ) )
{
								keyback.send ( ) ;
}
...
defaultRel ( & spaceback, ! bX, ! bY, ! bZ ) ;
						...
						if ( ! trigger ( & spaceback, & InternalMods ) )
{
							spaceback.send ( ) ;
}
						bRel = bX = bY = bZ = false ;

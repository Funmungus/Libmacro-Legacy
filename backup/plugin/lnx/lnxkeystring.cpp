...
{
	std::vector < int > KeyString::defaultKeyValues ( )
	{
		std::vector < int > ret =
		{
			0, //Null character
	...
	std::vector < bool > KeyString::defaultShiftedValues ( )
	{
		std::vector < bool > ret =
		{
	...
	std::map < std::pair < int, bool >, char > KeyString::defaultCharMap ( )
	{
		std::map < std::pair < int, bool>, char > ret ;
		auto keys = defaultKeyValues ( ) ;
		auto shifts = defaultShiftedValues ( ) ;
		for ( char i = 0 ;
			( unsigned char ) i < keys.size ( ) && ( unsigned char ) i < shifts.size ( ) ;
			i++ )
		{
			// map key + shift-> character
			ret.insert ( std::pair < std::pair < int, bool >, int >
						( std::pair < int, bool > ( keys [ i ], shifts [ i ] ),
						i ) ) ;
		}
		return ret ;
	}

	std::vector < int > KeyString::_keyVals = defaultKeyValues ( ) ;
	std::vector < bool > KeyString::_needShift = defaultShiftedValues ( ) ;
	std::map < std::pair < int, bool >, char > KeyString::_charMap = defaultCharMap ( ) ;
}

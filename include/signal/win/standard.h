...
# define MCR_KEY_SCAN( key ) \
	( key ).scan
# define MCR_KEY_SET_SCAN( key, scanCode ) \
	( key ).scan = ( scanCode )
# define MCR_KEY_UP_TYPE( key ) \
	( key ).up_type
# define MCR_KEY_SET_UP_TYPE( key, upType ) \
	( key ).up_type = ( upType )
# define MCR_KEY_QUICKSEND( key ) \
	if ( ( key ).up_type != MCR_UP ) \
	{ \
		keybd_event ( ( BYTE ) ( key ).key, ( BYTE ) ( key ).scan, \
...
# define MCR_KEY_SEND( key, success ) MCR_KEY_QUICKSEND ( key )

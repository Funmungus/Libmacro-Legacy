...
{
	unsigned int *found = mcr_Map_value(&ctx->signal.map_name_modifier, &name);
	return found ? *found : MCR_MF_NONE;
}

const char *mcr_ModFlags_name(struct mcr_context *ctx, unsigned int modifier)
{
	mcr_String *ret = mcr_Map_value(&ctx->signal.map_modifier_name, &modifier);
	return ret ? ret->array : NULL;
}

size_t mcr_ModFlags_count(struct mcr_context * ctx)
{
	return ctx->signal.map_modifier_name.set.used;
}
					   ...
					   struct mcr_Map **mapNameMod)
{
	dassert(ctx);
	if (mapModName)
		*mapModName = &ctx->signal.map_modifier_name;
	if (mapNameMod)
		*mapNameMod = &ctx->signal.map_name_modifier;
}

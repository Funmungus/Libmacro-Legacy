...
static void mcr_StringSet_deinit_range(mcr_StringSet * setPt,
									   size_t firstIndex,
									   size_t lastIndex);
						 ...
						 const char **strArr, size_t count)
{
	...
	mcr_String_init(&initial);
	if (mcr_Array_insert(setPt, pos, NULL, count) ||
		mcr_Array_fill(setPt, pos, &initial, count)) {
		return mcr_err;
	}
	...
	if (strArr) {
		i = count;
		while (i--) {
			if (mcr_String_replace(MCR_STRINGSET_ELEMENT
...
void mcr_StringSet_remove_index(mcr_StringSet * setPt, size_t pos,
								size_t count)
{
	dassert(setPt);
						  ...
						  const char **arraySource, size_t count)
{
	...
	/* Frees if reducing */
	if (mcr_StringSet_resize(setPt, count))
		return mcr_err;
					   ...
					   const char **srcArray, size_t count)
{
	...
	if (!count)
		return 0;
	if (mcr_StringSet_minused(dstPt, dstPos + count))
		return mcr_err;
	if (srcArray) {
		i = count;
		while (i--) {
...
int mcr_StringSet_set(mcr_StringSet * setPt, size_t pos, const char *copyStr)
{
	...
	if (mcr_StringSet_minused(setPt, pos))
		return mcr_err;
	if (copyStr) {
					   ...
					   size_t count, const char *copyStr)
{
	...
	size_t bytes;
	dassert(setPt);
	if (mcr_StringSet_minused(setPt, pos + count))
		return mcr_err;
	mcr_Array_iter_range(setPt, &itPt, &end, &bytes, pos, pos + count);
	while (itPt < end) {
		if (mcr_String_replace((mcr_String *) itPt, copyStr))
			return mcr_err;
		itPt += bytes;
	}
	return 0;
					  ...
					  const char **copyStr, size_t count, bool flagUnique)
{
	...
	if (!count)
		return 0;
	if (mcr_StringSet_smartsize(setPt, count))
		return mcr_err;
	if (flagUnique) {
		dassert(copyStr);
		for (i = 0; i < count; i++) {
			if (!(found = mcr_StringSet_find(setPt, copyStr[i]))) {
				if (mcr_StringSet_set(setPt, setPt->used,
									  copyStr[i])) {
					return mcr_err;
				}
	...
	if (mcr_StringSet_append(setPt, copyStr, count))
		return mcr_err;
	mcr_StringSet_sort(setPt);
	return 0;
}

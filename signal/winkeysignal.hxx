...
namespace funlibrary
{
	...
	bool KeySignal::send ( )
	{
		DWORD upFlag = ( getKeyUp ( ) == KeyUpType::UP ) ? KF_UP : 0 ;

		// First parameter is key code entry. VKey must be 1 to 254
		if ( ( unsigned int) getKey ( ) >= 0xFF )
			return false ;

		keybd_event ( ( BYTE ) getKey ( ), ( BYTE ) getScan ( ), upFlag, 0 ) ;

		if ( getKeyUp ( ) == KeyUpType::BOTH )
		{
			upFlag = KF_UP ;
			keybd_event ( ( BYTE ) getKey ( ), ( BYTE ) getScan ( ), upFlag, 0 ) ;
		}

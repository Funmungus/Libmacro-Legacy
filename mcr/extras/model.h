...
struct MCR_API CtxISignal {
	...
	CtxISignal(Libmacro *context, mcr_signal_fnc send,
		   ...
		   const Interface &interface = mcr_Interface_new(0, NULL, NULL, NULL,
						NULL)) throw(int);
	CtxISignal(mcr_signal_fnc send) throw(int);
	virtual ~CtxISignal() {}
...
struct MCR_API CtxDispatcher {
	...
	CtxDispatcher(Libmacro *context = NULL, mcr_Dispatcher_add_fnc add = NULL,
		      ...
		      mcr_Dispatcher_fnc trim = NULL) throw(int);
...
struct MCR_API Signal {
	...
	Signal(const Signal &copytron) throw(int)
	{
		int err;
		mcr_Signal_init(ptr());
	...
	/*! \brief \ref mcr_Signal_copy */
	Signal(const mcr_Signal &copytron) throw(int)
	{
		int err;
		mcr_Signal_init(ptr());
		if ((err = mcr_Signal_copy(ptr(), &copytron)))
	...
	/*! \brief \ref mcr_Signal_copy */
	inline Signal &operator =(const Signal &copytron) throw(int)
	{
		return operator =(copytron.ptr());
	}
	/*! \brief \ref mcr_Signal_copy */
	inline Signal &operator =(const mcr_Signal &copytron) throw(int)
	{
	...
	inline Signal &operator =(mcr_ISignal *isignal) throw(int)
	{
		setISignal(isignal);
		return *this;
	}
	...
	/*! \brief \ref mcr_Signal_copy */
	void copy(const mcr_Signal *copytron) throw(int)
	{
		int err;
		if (copytron != ptr()) {
			if ((err = mcr_Signal_copy(ptr(), copytron)))
	...
	void setISignal(mcr_ISignal *isignal) throw(int);
};
...
struct MCR_API Trigger {
	...
	Trigger(const Trigger &copytron) throw(int)
	{
		int err;
		mcr_Trigger_init(ptr());
	...
	/*! \brief \ref mcr_Trigger_copy */
	Trigger(const mcr_Trigger &copytron) throw(int)
	{
		int err;
		mcr_Trigger_init(ptr());
		if ((err = mcr_Trigger_copy(ptr(), &copytron)))
	...
	/*! \brief \ref mcr_Trigger_copy */
	inline Trigger &operator =(const Trigger &copytron) throw(int)
	{
		return operator =(copytron.ptr());
	}
	/*! \brief \ref mcr_Trigger_copy */
	inline Trigger &operator =(const mcr_Trigger &copytron) throw(int)
	{
		return operator =(&copytron);
	}
	/*! \brief \ref mcr_Trigger_copy */
	...
	inline Trigger &operator =(mcr_ITrigger *itrigger) throw(int)
	{
	...
	/*! \brief \ref mcr_Trigger_copy */
	void copy(const mcr_Trigger *copytron) throw(int)
	{
		int err;
		if (copytron != ptr()) {
			if ((err = mcr_Trigger_copy(ptr(), copytron)))
	...
	/*! \brief \ref mcr_Trigger_deinit + set \ref mcr_Trigger.itrigger */
	void setITrigger(mcr_ITrigger *itrigger) throw(int);
};
...
struct MCR_API Macro {
	...
	Macro(Libmacro *_context, bool block, bool sticky, unsigned int threadMax,
	      bool enable) throw(int);
	virtual ~Macro() throw(int);
	Macro(const Macro &copytron) throw(int);
	Macro(const mcr_Macro &copytron) throw(int);
	Macro(const mcr_Macro *copytron) throw(int);
	inline Macro &operator =(const Macro &copytron) throw(int)
	{
		copy(copytron);
		return *this;
	}

	inline Macro &operator =(const mcr_Macro &copytron) throw(int)
	{
		copy(&copytron);
		return *this;
	}
	...
	void setInterruptor(Interrupt val) throw(int);
	...
	void copy(const Macro &copytron) throw(int)
	{
		copy(copytron.ptr());
	}
	void copy(const mcr_Macro *copytron) throw(int)
	{
		int err;
		if (copytron != &_macro) {
...
struct MCR_API Stage {
	...
	Stage(Libmacro *ctx = NULL,
	      ...
	      unsigned int mods = 0, int trigFlags = MCR_TF_ALL) throw(int);
	~Stage() throw(int);
	Stage(const Stage &copytron) throw(int);
	Stage(const mcr_Stage *copytron) throw(int);
	Stage &operator =(const Stage &copytron) throw(int)
	{
		_context = copytron._context;
		copy(copytron.ptr());
		return *this;
	}
	Stage &operator =(const mcr_Stage *copytron) throw(int)
	{
		copy(copytron);
		return *this;
	}
	...
	void setIntercept(mcr_Signal *val) throw(int);
	void setGenericIntercept(mcr_Signal *val) throw(int);

	int compare(const mcr_Stage *rhsPt)
	{
		return mcr_Stage_compare(ptr(), rhsPt);
	}
	void copy(const mcr_Stage *copytron) throw(int)
	{
		int err;
		if (copytron != ptr()) {

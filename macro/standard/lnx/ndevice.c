...
int mcr_Device_enable(struct mcr_Device *devPt, bool enable)
{
	...
	if ((mtxErr = mtx_lock(&_deviceLock)) != thrd_success) {
		err = mcr_thrd_errno(mtxErr);
		mset_error(err);
		return err;
	}
	/* Start by opening. */
	if ((err = device_open(devPt)))
		return mtx_unlock_error(mtxErr, err);
	/* Force evbit satisfaction. */
	if ((err = device_write_evbit(devPt)))
		return mtx_unlock_error(mtxErr, err);
	/* Then write all other bits. Ensured at least one mapping pair. */
	if ((err = device_write_non_evbit(devPt)))
		return mtx_unlock_error(mtxErr, err);
	if ((err = device_create(devPt)))
		return mtx_unlock_error(mtxErr, err);
...
static int device_open_event(struct mcr_Device *devPt)
{
	...
	if ((err = device_open_event_wd(devPt) || chdir(wd) == -1)) {
		free(wd);
		err = errno;
		if (!err)
			err = EINTR;
...
static int device_write_non_evbit(struct mcr_Device *devPt)
{
	...
	while (itPt < end) {
		...
		if (evBit != UI_SET_EVBIT) {
			...
			if ((err = device_write_bits(devPt, evBit, evbitArr)))
				return err;
		}
		itPt += bytes;
	}
...
{
	int err = 0, mtxErr = mtx_init(&_deviceLock, mtx_plain);
	UNUSED(context);
	if (mtxErr != thrd_success) {
		err = mcr_thrd_errno(mtxErr);
		mset_error(err);
	...
	if ((err = mcr_String_replace(&_uinputPath, MCR_STR(MCR_UINPUT_PATH))))
		return err;
	if ((err = mcr_String_replace(&_eventPath, MCR_STR(MCR_EVENT_PATH))))
		return err;
	/* ioctl is unpredictable for valgrind. The first ioctl will
	 * be uninitialized, and others should not cause errors. */
	if ((err = genDevice_init()))
		return err;
	if ((err = absDevice_init()))
		return err;
